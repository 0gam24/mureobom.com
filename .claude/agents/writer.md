...
keyPoints:                     # "한눈에 핵심" 박스 (신규 글 필수 작성, 3~5개 권장)
  - "{핵심 사실 1 — 정의·자격 등, 8~120자, 완결 문장}"
  - "{핵심 사실 2 — 금액·비율 등 숫자 1개 이상 포함}"
  - "{핵심 사실 3 — 시행 시기·주의점 등}"
updated: 2026-05-19
...
| (신규) — | `keyPoints` | "한눈에 핵심" 불릿 3~5개. summary와 중복 문장 금지, 각 불릿은 독립 사실(정의·금액·자격·시기) 1개씩. 본문 내용에서만 뽑는다 |
...
2. **본문 Q&A** — brief의 `must_cover` 각 항목을 `##` 소제목 + 답 단위로.
   - `##` 소제목은 **질문형 완결 문장**으로 (페이지가 H2를 자동 수집해 목차를
     렌더하므로 소제목만 읽어도 글 구조가 보여야 한다). **본문에 목차 섹션을
     직접 만들지 마라** — 중복 노출된다. "한눈에 핵심"도 keyPoints 프론트매터로만
     표현하고 본문에 같은 박스를 또 만들지 않는다.
...
# 문체 — AI 생성 티 제거 (compliance W-01·W-02)

긴 줄표와 반복 상투구는 대표적인 AI 문체 신호다. AdSense 심사·검색 품질평가에서
"AI로 찍어낸 사이트" 인상을 주므로 발행 노출 텍스트에서 제거한다.

- **긴 줄표(— em dash, – en dash) 사용 금지** — `title`·`summary`·본문·`faq`·
  인포그래픽 SVG 텍스트 전부. 쉼표·가운뎃점(`·`)·괄호·문장 분리로 대체한다.
  - ❌ `교육비 세액공제 2026 — 자녀 300만`
  - ✅ `교육비 세액공제 2026, 자녀 300만 대학 900만`
  - compliance W-01이 신규 글에서 차단한다. (자동화 내부 파일·주석은 적용 제외)
- **상투구 반복 금지(W-02)**: `핵심은 세 가지`, `정리하면`, `결론부터 말하면`,
  `한눈에 정리` 같은 문구를 매 글 똑같이 반복하지 마라. 같은 글 구조(리드·표·FAQ)
  라도 도입·전환·마무리 문장 패턴은 글마다 바꾼다. 사람이 그때그때 쓰는 것처럼.
...
8. **발행 노출 텍스트에 긴 줄표(`—`·`–`)** — 쉼표·가운뎃점·괄호로 대체.
   compliance(W-01)가 차단. 상투구 매 글 반복도 금지(W-02).
