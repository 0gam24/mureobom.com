...
- 오늘 KST 날짜는 **`date -u -d '+9 hours' +%F`**로 구해 변수로 두고 사용한다
  (UTC에 +9시간 직접 계산 → 클라우드 Linux·로컬 Windows 양쪽 안전).
  프론트매터 `published`·`updated`, 커밋 메시지 날짜, 보고서 날짜 모두 이 값.
- 피해야 할 두 함정: 클라우드는 UTC 시계라 그냥 `date`면 새벽에 하루 밀리고
  (06:00 KST = 전날 21:00 UTC), 윈도우 Git Bash는 zoneinfo가 없어
  `TZ=Asia/Seoul date`가 UTC로 폴백한다. 그래서 위 `date -u -d '+9 hours'`로 통일.
- 자동 배포 스케줄은 매일 **06:00 KST**(cron `0 21 * * *` UTC — 2026-07-23 04:30에서
  변경) 발화. 한국은 서머타임이 없어 이 매핑은 연중 고정. 파이프라인은 발화 후 약
  30~60분 걸리므로 완료는 06:30~07:00 KST.
