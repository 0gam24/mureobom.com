...
## 날짜·시간 기준 — 항상 KST(Asia/Seoul), 예외 없음

이 사이트의 **모든 날짜·시간은 한국시간(KST) 기준**이다. 발행 프론트매터
(`published`·`updated`), 커밋 메시지 날짜, 보고서 날짜, 자동 배포 스케줄 전부.

- 오늘 날짜는 반드시 `TZ=Asia/Seoul date +%F`로 구한다. **그냥 `date`(UTC) 금지.**
- **클라우드(스케줄) 실행 환경은 UTC 시계**라, `date`를 쓰면 새벽 발행이 하루 빨리
  찍힌다(04:30 KST = 전날 19:30 UTC). 로컬 운영자 PC는 KST라 정상이지만 습관적으로
  `TZ=Asia/Seoul`을 붙인다. (과거 D-2026-07-11 실측: 클라우드 첫 런이 UTC로 찍혀 7/10로
  하루 밀렸고, KST 강제 후 7/12부터 정상.)
- 자동 배포 스케줄은 매일 **04:30 KST**(claude.ai routine, cron `30 19 * * *` UTC) 발화.
  한국은 서머타임이 없어 이 매핑은 연중 고정. 파이프라인은 발화 후 약 30~60분 걸리므로
  완료(커밋·라이브)는 대략 05:00~05:30 KST. 그 전에 확인하면 "아직 안 됨"이 정상이다.
...
- **단일 소스**: [src/styles/tokens.css](src/styles/tokens.css)가 색·간격·타이포의
  ...
  - **예외 — `public/diagrams/*.svg` 인포그래픽**: 단독 SVG 파일은 CSS 변수를
    상속받지 못하므로 표준 팔레트 hex를 **직접 기입**한다. 허용 값은 다음으로 고정:
    `#FBF7F0`·`#F4EEE2`·`#1A2B2A`·`#4A5856`·`#8A938F`·`#0E7C72`·`#0A5F58`·`#E3F0EE`·`#C2873B`·`#E3DCCF`·`#B5462F`.
    이 목록 밖의 색은 compliance V-02가 차단.
...
| 발행 텍스트의 긴 줄표 `—`·`–` | 대표적인 AI 문체 신호(AdSense 심사 리스크). 쉼표·가운뎃점(`·`)·괄호로 대체. 신규 글부터 compliance W-01이 차단, 기발행 제목은 소급 금지(재색인 손해) |
| `핵심은 N가지`·`정리하면` 반복 | 매 글 같은 상투구 = AI 티. 글마다 표현 다양화 (W-02) |
...
- **`sources` 0건이면 빌드 차단** — [src/content/config.ts](src/content/config.ts)
  ...
  이 가드를 우회하지 마라.
- **작성 주체 바이라인 + 검수 프로세스 투명화** —
  [src/pages/\[cluster\]/\[slug\].astro](src/pages/[cluster]/[slug].astro)가
  "{author} 작성 · 공식 1차 출처 대조 검수 / 발행·최종확인·근거 N건 ·
  이 글은 어떻게 만드나요?(→[/about/#how](src/pages/about.astro))" 바이라인과
  JSON-LD Article `citation`(sources)·`inLanguage`·`isAccessibleForFree`를 자동 렌더한다.
  E-E-A-T는 **정직한 편집부 + 검수 프로세스**로만 구현 — 실재하지 않는 개인 전문가·
  자격 사칭은 금지(compliance E-02). [/about/#how](src/pages/about.astro) 방법론
  섹션(검수 절차·비자문 고지)은 이 바이라인이 가리키는 필수 신뢰 장치이므로 제거·앵커
  변경 금지.
...
- **인포그래픽 1개 이상 필수**. 신규 글은 `public/diagrams/{slug}.svg` 인포그래픽을
  **최소 1개** 본문에 `![alt](/diagrams/{slug}.svg)`로 포함한다. 정보 전달용
  ...
  (아래 디자인 시스템 규칙 참조). compliance V-01(존재)·V-02(팔레트)가 강제하므로
  이 가드를 우회하지 마라.
...
신뢰·시각 신호도 compliance가 강제한다: `E-*`(작성 주체 정직성 — 개인 전문가·자격
사칭 금지, 바이라인·/about/#how 방법론 존재)·`V-*`(인포그래픽 1개 이상 존재 + 브랜드
팔레트 hex 준수)·`W-*`(문체 — 발행 텍스트 긴 줄표 `—`·`–` 금지 + 상투구 반복 금지,
신규 글만·기발행 소급 금지). 룰 정의는 [compliance/checklist.md](compliance/checklist.md).
