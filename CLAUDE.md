...
- 오늘 KST 날짜는 **`date -u -d '+9 hours' +%F`**로 구한다(UTC에 +9시간 직접 계산).
  이 방식만 **클라우드 Linux·로컬 Windows Git Bash 양쪽에서 안전**하다.
  - ✗ 그냥 `date +%F`: 클라우드(UTC 시계)에서 새벽에 하루 밀린다(04:30 KST = 전날 19:30 UTC).
  - ✗ `TZ=Asia/Seoul date +%F`: 윈도우 Git Bash엔 zoneinfo가 없어 **UTC로 폴백**한다(로컬 함정).
