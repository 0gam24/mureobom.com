...
| V-05 | 신규 글은 대표 이미지 `public/img/{slug}.webp`(1200×900 WebP, ≥5,000B) + 프론트매터 `hero` 필수. `scripts/gen_post_hero.mjs --slug`가 본문 첫 인포그래픽을 래스터화해 생성 — og:image·Article.image·primaryImageOfPage·이미지 사이트맵·RSS enclosure의 1순위(검색 썸네일). OG PNG(`image`)는 폴백. (기존 글은 2026-09-02 백필 완료) | 차단 |
...
| W-02 | 상투구 반복 금지 — `핵심은 N가지`, `정리하면`, `결론부터 말하면`, `한눈에 정리`, `첫째/둘째`, `결론적으로` 류 표현을 모든 글에서 동일하게 반복하지 말 것. 고정 소제목(`정리 — 한 줄로`, `흔한 오해`)의 전 글 반복도 포함. 같은 글 구조(리드·표·FAQ)라도 문장 패턴은 글마다 다양화 | 경고 |
| W-03 | **신규 글** 본문에 화살표(`→`) 단계 표현 2회 이상 금지 (google-content-master-prompt-v4 PART 22). 과정·인과는 문장으로 | 차단 |
| W-04 | 단일 기능 블록 — 체크리스트·확인사항·핵심 정리·마지막 점검 성격 블록은 글 전체 1개 이하. 결론에서 본문 금액·기관명 재나열 금지. FAQ Q가 본문 H2 복사 금지 (PART 18~21·31) | 경고 |
